...
if __name__ == '__main__':
  ...
  parser.add_argument('', '--pickle_path', default=None, dest='pickle_path',
                    help='Point to an alternative records.pickle file.')
  parser.add_argument('', '--ids_filter', default='', dest='ids_filter',
                    help='Comma-separated list of Photo IDs to consider.')
  parser.add_argument('', '--previous_geocode_json', default='',
  ...
  parser.add_argument('-c', '--coders', dest='ok_coders', default='all',
                    help='Set to a comma-separated list of coders')

  parser.add_argument('-g', '--geocode', dest='geocode', action='store_true',
  ...
  parser.add_argument('-n', '--use_network', dest='use_network',
  ...
  parser.add_argument('-p', '--print_records', dest='print_recs',
                    action='store_true', type=bool, default=False,
                    help='Set to print out records as they\'re coded.')
  parser.add_argument('-o', '--output_format', default='',
                    help='Set to either lat-lons.js, lat-lons-ny.js, records.js'
                    ' or entries.txt to output one of these formats.')
  parser.add_argument('', '--lat_lon_map', default='', dest='lat_lon_map',
  ...
  if args.ok_coders != 'all':
    ok_coders = args.ok_coders.split(',')
    geocoders = [c for c in geocoders if c.name() in ok_coders]
    if len(geocoders) != len(ok_coders):
      sys.stderr.write('Coder mismatch: %s vs %s\n' % (args.ok_coders, ','.join([c.name() for c in geocoders])))
      sys.exit(1)

  # TODO(danvk): does this belong here?
  lat_lon_map = {}
  if args.lat_lon_map:
    for line in file(args.lat_lon_map):
      line = line.strip()
      if not line: continue
      old, new = line.split('->')
      lat_lon_map[old] = new

  rs = record.AllRecords(path=args.pickle_path)
  if args.ids_filter:
    ids = set(args.ids_filter.split(','))
    rs = [r for r in rs if r.photo_id() in ids]

  # Load existing geocodes, if applicable.
  id_to_located_rec = {}
  if args.previous_geocode_json:
    prev_recs = json.load(file(args.previous_geocode_json))
  ...
  elif args.output_format == 'records.js':
